...
def command_plan() -> list[dict]:
    ...
    # What-if commands deliberately enable conditional resources so Azure validates
    # the proposed graph without creating anything.
    vm_parameters = [
        "--parameters",
        "provisionPilot=true",
        "suffix=${PILOT_SUFFIX}",
    ...
    steps = [
        {
            ...
            "argv": ["az", "rest", "--method", "get", "--url", "${RETAIL_PRICE_URL}"],
            "capture_stdout_to": "${LIVE_PRICE_EVIDENCE}",
            "verify_argv": [
        ...
        {
            "id": 3,
            "name": "check_t4_capacity",
            "argv": [
                "az", "deployment", "group", "what-if",
                "--resource-group", "${PILOT_RESOURCE_GROUP}",
                "--template-file", "infra/three-family-pilot-vm.bicep",
                *vm_parameters,
            ],
            "mode": "what_if",
        },
        {
            "id": 4,
            "name": "provision_independent_watchdog",
            "argv": [
                "az", "deployment", "group", "what-if",
                "--resource-group", "${WATCHDOG_RESOURCE_GROUP}",
                "--template-file", "infra/three-family-watchdog.bicep",
                *watchdog_parameters,
            ],
            "mode": "what_if",
        },
        {
            "id": 5,
            "name": "create_single_standard_nc4as_t4_v3",
            "argv": [
                "az", "deployment", "group", "what-if",
                "--resource-group", "${PILOT_RESOURCE_GROUP}",
                "--template-file", "infra/three-family-pilot-vm.bicep",
        ...
        {
            "id": 7,
            "name": "download_and_hash_verify_allowlist",
            "argv": [
                "python3", "-m", "training.snapshot_verifier",
                "--all-families", "--root", "${MODEL_SNAPSHOT_ROOT}",
            ],
            "mode": "validate_only_no_download",
        },
        {
            "id": 8,
            "name": "run_compatibility_probes",
            "argv": [
                "python3", "-m", "training.three_family_contract",
                "--probe-evidence", "${PROBE_EVIDENCE}",
            ],
        ...
        {
            "id": 16,
            "name": "deallocate_and_delete_everything",
            "argv": ["az", "group", "delete", "--name", "${PILOT_RESOURCE_GROUP}", "--yes", "--no-wait"],
            "watchdog_argv": ["az", "group", "delete", "--name", "${WATCHDOG_RESOURCE_GROUP}", "--yes", "--no-wait"],
            "mode": "print_only_destructive",
        },
        {
            "id": 17,
            "name": "confirm_zero_residual_resources",
            "argv": ["az", "resource", "list", "--resource-group", "${PILOT_RESOURCE_GROUP}", "--output", "json"],
            "watchdog_argv": ["az", "resource", "list", "--resource-group", "${WATCHDOG_RESOURCE_GROUP}", "--output", "json"],
            "group_exists_argvs": [
